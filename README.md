...
## 6. Alignement des données et leur datavisualisation 
Nous avons regrouper deux fichiers de données, celui de population et celui avec les prix au m2 pour voir si ces deux mouvements sont liés. En sélectionnant la population ,ainsi que le prix moyen, nous pouvons comparer l'un avec l'autre.Par défaut, vous vouez toutes les années et tous prix. Pour faire une comparaison, il est néccessaire de cliquer sur "Sélectionner les données à comparer", ensuite choisir l'année qui vous intéresse.
